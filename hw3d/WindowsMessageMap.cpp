...
std::wstring WindowsMessageMap::operator()(DWORD msg, LPARAM lp, WPARAM wp) const noexcept
{
	...
	{
		woss << std::left << std::setw(firstColWidth) << L"Unknown message: 0x" << std::hex << msg << std::right;
	}

	woss << L"   LP: 0x" << std::hex << std::setfill(L'0') << std::setw(8) << lp;
	woss << L"   WP: 0x" << std::hex << std::setfill(L'0') << std::setw(8) << wp << std::endl;
